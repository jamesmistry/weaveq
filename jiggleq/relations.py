"""@package relations
Classes for expressing the relationships between fields that override Python operators to provide a simple API and for ease of parsing.
"""
...
class F(object):
    """Represents a field relationship expression
    ...
    def __init__(self, name, proxy = None):
        """Constructor. If no field proxy is specified, a default transparent one is used."""
        self.name = name
        self.tree = None
...
class ConditionNode(object):
    ...
    def leaf(self):
        """Is this node a leaf node (i.e. does it have exactly 0 children)?"""
        return (len(self._edges) == 0)

    def leaves(self):
        """Return a list of leaf nodes in the sub-tree represented by this node."""
        leaves = []
